...
public class CartPopUpFromOrder extends JFrame {
	...
	// necessario per l'aggiornamento del model della tabella dei libri e per il cart
	private CheckOutView cv;
	private ArrayList<Book> cart = new ArrayList<Book>();
	
	private int selectedRow;
	
	public CartPopUpFromOrder(ArrayList<Book> cart, CheckOutView cv) {
		this.cart = cart;
		this.cv = cv;
		initialize();
	}
	...
	public void clearCart() {
		...
		CartPopUpFromOrder temp = new CartPopUpFromOrder(cart, cv);
		CheckOutView mp = new CheckOutView(cart, cv.getUser(), cv.getBookList());
		cv.dispose();
		mp.setInfo();
		mp.setVisible(true);
		this.dispose();
